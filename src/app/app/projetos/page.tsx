...
export default async function ProjectsPage() {
  ...
  const [{ data: projects, error }, { data: leads }] = await Promise.all([
    supabase
      .from("projects")
      .select("id, name, status, value, due_on, lead_id, project_tasks(id, title, status, priority, due_at)")
      .eq("workspace_id", workspaceId)
      .order("created_at", { ascending: false }),
    supabase.from("leads").select("id, company_name").eq("workspace_id", workspaceId).order("company_name"),
  ]);
      ...
      {!error && (
        <>
          <form action={createProject} className="app-card mt-8 grid gap-3 p-5 md:grid-cols-5">
            ...
            />
            <select name="leadId" className={fieldClass}>
