...
export function WhatsAppChatModal({ leadId, companyName, phone, open, onClose }: WhatsAppChatModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/35 p-4 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="wa-chat-title">
      <div className="flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[0_20px_60px_rgba(15,23,42,0.18)]">
        ...
        <div className="flex items-center gap-2 border-b border-[var(--border)] px-4 py-2">
          <button
            ...
            onClick={toggleAgent}
            className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border)] px-2.5 py-1.5 text-xs font-semibold text-[var(--text-2)] hover:bg-[var(--neu-bg-well)] disabled:opacity-50"
          >
            <Robot size={14} />
        ...
        <div className="flex-1 space-y-2 overflow-y-auto bg-[var(--neu-bg-pop)] px-4 py-4" aria-busy={loading || busy}>
          ...
          {!loading && !loadError && messages.length === 0 && (
            <p className="rounded-xl border border-dashed border-[var(--border)] bg-[color-mix(in_oklab,var(--card)_70%,transparent)] px-3 py-6 text-center text-xs text-[var(--text-4)]">
                ...
                <div
                  className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm leading-5 ${
                    message.direction === "outbound"
                      ...
                      : "rounded-bl-md border border-[var(--border)] bg-[var(--card)] text-[var(--text)]"
        ...
        <form onSubmit={send} className="border-t border-[var(--border)] p-3" aria-busy={busy}>
          ...
          <div className="flex gap-2">
            <input
              ...
              className="h-11 flex-1 rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 text-sm text-[var(--text)] outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/15 disabled:opacity-60"
            />
            <button
              type="submit"
              disabled={busy || loading || !text.trim()}
      ...
      <ul className="mt-4 divide-y divide-[var(--border)]">
        ...
        {conversations.slice(0, 8).map((item) => (
          <li key={item.id} className="flex items-center justify-between gap-3 py-3 text-sm">
            ...
            <span className={`shrink-0 rounded-md border px-2 py-1 text-[11px] font-semibold ${item.agent_enabled ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-700" : "border-[var(--border)] bg-[var(--neu-bg-well)] text-[var(--text-3)]"}`}>
