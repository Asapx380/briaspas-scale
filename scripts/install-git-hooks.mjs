...
// Always pin this clone to Wesley so cloud-agent globals cannot leak
// (GitHub squash-merge copies extra authors into Co-authored-by trailers).
execSync('git config user.name "Wesley Luther"', { cwd: root });
execSync('git config user.email "wesleyluther830@gmail.com"', { cwd: root });
console.log(
  "Local user.name/email pinned to Wesley Luther <wesleyluther830@gmail.com>.",
);
